...
export default function AdminDashboardScreen() {
  ...
  // Render an active booking item
  const renderActiveBookingItem = (booking: Booking) => (
    <View 
      key={booking.id} 
  ...
  // Render recent booking table row
  const renderRecentBookingRow = (booking: Booking) => (
    <View 
      key={booking.id}
    ...
    <>
      ...
      <View style={styles.lotsSection}>
        ...
        {parkingLots.length === 0 ? (
          ...
          // Change FlatList to ScrollView for horizontal scrolling to fix nesting issue
          <ScrollView
            horizontal
          ...
          >
            {parkingLots.map(item => (
              <View 
                key={item.id}
                style={[styles.lotCard, { backgroundColor: isDarkMode ? 'rgba(255,255,255,0.05)' : colors.cardBackground }]}
              >
      ...
      <View style={styles.bookingsSection}>
        ...
        {activeBookings.length === 0 ? (
          ...
          activeBookings.slice(0, 3).map(renderActiveBookingItem)
        )}
      ...
      <View style={[styles.bookingsSection, { paddingBottom: spacing.xl }]}>
        ...
        {recentBookings.length === 0 ? (
          ...
          <View style={[styles.recentBookingsCard, { backgroundColor: isDarkMode ? 'rgba(255,255,255,0.05)' : colors.cardBackground }]}>
            ...
            {recentBookings.map(renderRecentBookingRow)}
          </View>
        )}
      </View>
    </>
